...
class HapticsManager: ObservableObject {
    ...
    func playRealtimeTouch(intensity: Float, sharpness: Float = 0.5) {
        ...
        let intensityRaw = UserDefaults.standard.string(forKey: "hapticIntensity") ?? "medium"
        let multiplier: Float = intensityRaw == "light" ? 0.5 : intensityRaw == "strong" ? 1.5 : 1.0
        let adjustedIntensity = intensity * multiplier
    ...
    func playConnectionEstablished() {
        ...
        do {
            ...
            print("Failed to play connection haptic: \(error)")
        }
    }
    
    private func createPattern(for gesture: HapticGesture) throws -> CHHapticPattern {
        let intensityRaw = UserDefaults.standard.string(forKey: "hapticIntensity") ?? "medium"
        let intensityMultiplier: Float = intensityRaw == "light" ? 0.5 : intensityRaw == "strong" ? 1.5 : 1.0
        let events = gesture.events.map { event in
            CHHapticEvent(
                eventType: event.type,
                parameters: [
                    CHHapticEventParameter(parameterID: .hapticIntensity, value: event.intensity * intensityMultiplier),
